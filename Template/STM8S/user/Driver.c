//Driver.c - CAFrame工程模板 驱动程序stm8s单片机示例 
//STM8S103F3 12M外部晶振
...
****************************************************************************/
static volatile uint16_t g_SysTick;
...
void drv_init(void)
{
	...
	//输出口初始化
	GPIOD->DDR|=(1<<4)|(1<<3);
	GPIOD->ODR&=~((1<<4)|(1<<3)); //初始输出低
	GPIOD->CR1|=(1<<4)|(1<<3);//推挽输出
	GPIOD->CR2|=(1<<4)|(1<<3);//10MHz
...
{
	uint16_t tmp=drv_get_current_tick();
	return tmp-t;
}
